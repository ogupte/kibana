...
export async function fetchServiceDependenciesFromTraceIds(
  setup: Setup,
  serviceName: string,
  traceIds: string[]
) {
        ...
        service_dependencies: {
          scripted_metric: {
            ...
            reduce_script: {
              ...
              def services = new HashSet();

              for(foundPath in context.paths) {
                if (!context.locationsToRemove.contains(foundPath)) {
                  // instead of taking the head of the path list, find the current service in the list, then keep the one right after
                  //services.add(foundPath[0]);
                  def count = 0;
                  while (count < foundPath.size()) {
                    if (foundPath[count]['service.name'] == serviceName) {
                      def dependencyNode = foundPath[count+1];
                      if (dependencyNode.containsKey('service.name')) {
                        services.add(dependencyNode);
                      }
                      break;
                    }
                    count++;

...
import { fetchServiceDependenciesFromTraceIds } from './fetch_service_dependencies_from_trace_ids';
...
export async function getServiceDependencies({
  ...
  // return ['opbeans-go', 'opbeans-java', 'opbeans-node', 'opbeans-ruby'];
  const { traceIds } = await getTraceSampleIds({
    setup,
    serviceName,
    environment: setup.uiFilters.environment,
  });
  return await fetchServiceDependenciesFromTraceIds(
    setup,
    serviceName,
    traceIds
  );

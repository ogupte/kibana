...
export async function serviceMapRun(kbnServer: any, config: any, lastRun?: string) {
  ...
  // );
  const apmIdxPattern = 'apm-*';
  const serviceConnsDestinationIndex = '';
  const serviceConnsDestinationPipeline = '';
  ...
  while (true) {
    ...
    const body = connDocs.map((connDoc: any) => JSON.stringify(connDoc)).join('\n');
    // console.log(body);
    console.log('#$#$#$#$#$#$#$#$#$#$#$#$');
